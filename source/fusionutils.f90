MODULE FUSIONUTILS
    ...
    DO WHILE(NPROC.LT.(TOTNPART-1).AND.SUM(PGROUP%NPART).GT.0)
      ...
      DO WHILE(NCHECK.LT.REMPART.AND.SUM(PGROUP%NPART).GT.0) 
        ...
        IF(CHECKFUSE) THEN
          ...
          IF(PARTP%NFUSE.EQ.1) THEN
            PARTP%XFUSE = PARTP%POS
            PARTP%TFUSE = PARAMP%CURTIME-PARTP%TMAKE
          END IF
  ...
  SUBROUTINE CHECKFUSION(CHECKFUSE,PARTP1,PARTP2,PARAMP)
    ...
    TYPE(PARTICLEPARAMS),POINTER :: PARAMP
    DOUBLE PRECISION :: POS1,POS2,PREVPOS1,PREVPOS2
    DOUBLE PRECISION :: VEC1,VEC2
    INTEGER :: PTYPE1,PTYPE2
    DOUBLE PRECISION :: PF,SEP,MAXSEP
    ...
    ! end check if either particle is incapable of fusion
    IF(.NOT.(PARTP1%CANFUSE.AND.PARTP2%CANFUSE)) RETURN
    ...
    ! end check if only one of the particles is in the tip
    IF(PARTP1%ISINTIP.NEQV.PARTP2%ISINTIP) RETURN
    ...
    IF(PARAMP%USEKFUSE) THEN
      PF = 1-EXP(-PARAMP%KFUSE(PTYPE1,PTYPE2)*PARAMP%DELT)
    ELSE
    ...
    ! use explicit interactions to determine fusion
    ! this is useful if the timestep is smaller than time required for crossover
    IF(.NOT.PARTP1%ISINTIP) THEN
      IF(PARAMP%USEINTERACTIONS) THEN
      ...
      ELSE
      ! determine fusion based on particle crossover
        IF(PARTP1%CURDIR*PARTP2%CURDIR.GE.0D0) THEN
          ! SAME DIRECTION
          MAXSEP = ABS(PARTP1%VEL-PARTP2%VEL)*PARAMP%DELT
        ELSEIF(PARTP1%CURDIR*PARTP2%CURDIR.LT.0D0)THEN
          ! different directions
          MAXSEP = (ABS(PARTP1%VEL)+ABS(PARTP2%VEL))*PARAMP%DELT
        END IF

        IF(SEP.GT.MAXSEP) RETURN
    ...
    IF(CHECKFUSE.AND..NOT.(PARTP1%CANFUSE.AND.PARTP2%CANFUSE)) THEN
      PRINT*, 'TESTXA: particles fused without flag'
      PRINT*, 'CANFUSE1, PTYPE1, NFUSE1: ',PARTP1%CANFUSE, PARTP1%PTYPE, PARTP1%NFUSE
      PRINT*, 'CANFUSE2, PTYPE2, NFUSE2: ',PARTP2%CANFUSE, PARTP2%PTYPE, PARTP2%NFUSE
      STOP 1
    END IF
    
    ! update interaction
    IF(PARAMP%USEINTERACTIONS) CALL UPDATEINTERACTION(PARTP1,PARTP2,.NOT.CHECKFUSE)
