...
CONTAINS  
  FUNCTION ONES(N)
	!generates an array of ones of size N
    INTEGER, INTENT(IN) :: N
  ...
  FUNCTION LINSPACE(XMIN,XMAX,N)
    ...
    INTEGER :: N,I
    DOUBLE PRECISION :: LINSPACE(N)
    IF(N.EQ.1) THEN
      LINSPACE = XMIN
    ELSE
      DO I = 1,N
        LINSPACE(I) = XMIN + (XMAX-XMIN)*(I-1)/(N-1)
      END DO
    END IF
  ...
  FUNCTION LOGSPACE(XMIN,XMAX,N)
    ...
    DOUBLE PRECISION :: XMIN,XMAX
    INTEGER :: N,I
    DOUBLE PRECISION :: LOGSPACE(N)
    IF(N.EQ.1) THEN
      LOGSPACE = XMIN
    ELSE
      DO I = 1,N
        LOGSPACE(I) = XMIN*(XMAX/XMIN)**(DBLE(I-1)/(N-1))
      END DO
    END IF
  ...
  SUBROUTINE GETPOISSONEVENTS(LAMBDA,NVAL)
  ! returns a randomly sampled number of events N from a poisson distribution
  ! lambda is the rate parameter (number of events per time interval)
    IMPLICIT NONE
    DOUBLE PRECISION,INTENT(IN):: LAMBDA
    INTEGER,INTENT(OUT):: NVAL
    INTEGER :: NCT
    DOUBLE PRECISION :: EXPVAL,UPROD
  ...
  END SUBROUTINE GETPOISSONEVENTS
